    ...
    def forward(self, inputs, targets):
        ...
        if self.k > 1:
            ...
            targets = torch.zeros_like(log_probs).scatter_(
                1, targets.unsqueeze(1), 1 - self.epsilon)
            targets += torch.zeros_like(log_probs).scatter_(1,
                                                            topk, self.epsilon / self.k)
        else:
            targets = torch.zeros_like(log_probs).scatter_(
...
class SoftEntropy(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        if self.input_prob:
            ...
            loss = (- F.softmax(targets, dim=1).detach()
                    * log_probs).mean(0).sum()
        return loss
...
class FocalLoss(nn.Module):
    def __init__(self, gamma=2, alpha=0.25):
        ...
        self.alpha = alpha
        self.gamma = gamma
        print('Initializing FocalLoss for training: alpha={}, gamma={}'.format(
...
class LabelRefineLoss(nn.Module):
    def __init__(self, lambda1=0.0):
        ...
        print('Initializing LabelRefineLoss for training: lambda1={}'.format(
            self.lambda1))
...
class FocalTopLoss(nn.Module):
    ...
    def masked_softmax_multi_focal(self, vec, targets=None, dim=1):
        ...
        min_values = sorted[torch.range(
            0, sorted.shape[0]-1).long(), sorted_cum_min_indices]
        min_values = min_values.unsqueeze(
            dim=-1) * ori_neg_exps.sum(dim=1, keepdim=True)
